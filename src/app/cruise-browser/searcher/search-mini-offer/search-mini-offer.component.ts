...
@Component({
  selector: 'app-search-mini-offer',
  templateUrl: './search-mini-offer.component.html',
  styleUrls: ['./search-mini-offer.component.scss']
})
export class SearchMiniOfferComponent implements OnInit, OnDestroy {
  ...
  errorLoadingLabels: boolean = false;
  ...
  InitData() {
    ...
    const sources = [
      ...
      this.adminService.GetOfertasActivas()
    ];

    this.dataSubscription = forkJoin(sources)
      .subscribe(
        ([screenInfo, offers]: any[]) => {
  ...
  // BuildOffers(offersResult: any) {
  //   var total = 5;

  //   var elemento: any = {
  //     idOferta: 446,
  //     destino: "10",
  //     fechaInicioSalida: "2021-12-15T00:00:00",
  //     fechaFinSalida: "2021-10-22T00:00:00",
  //     company: "SBN",
  //     puertoSalida: null,
  //     nnoches: null,
  //     barco: null,
  //     descOferta: "Vancouver | Juneau | Skagway | Icy Strait Point | Glacier Bay | Sitka | Endicott Arm | Ketchikan | Prince Rupert | Victoria | Vancouver",
  //     tituloOferta: "Alaska 12 notti in Grand Princess",
  //     urlOfertaImg: "https://lasarenastest.istinfor.com/InfoBarcos/Agencias/000190-000190/OFERTAS/1.jpg",
  //     urlCompanyImg: this.filtersInfo.companias[1].image
  //   };

  //   for (let index = 0; index < total; index++) {
  //       this.offerts.push(elemento);
  //   }
  // }
      ...
      .subscribe(
        ([filtros, totalCount]: any[]) => {
          ...
          if (this.firstCall == true) {
            ...
            // if (offersResult && offersResult.length > 0)
            // {
            // this.BuildOffers(this.offersResult);
            // }
  ...
  SelectOffert(offert: any) {
    if (!this.navHaveChange) {
      this.currentFilters =
      {
        destino: offert.destino ? [offert.destino] : [],
        fecha: this.BuildDate(offert.fechaInicioSalida),
        puerto: offert.puertoSalida ? [offert.puertoSalida] : [],
        barco: offert.barco,
        companias: offert.company ? [offert.company] : [],
        duracion: offert.nnoches,
        orderBy: +this.orderBy,
        priceFilter:
        {
          minPrice: 1,
