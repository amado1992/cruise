...
@Component({
  ...
  styleUrls: ["./search-mini.component.scss"]
})
export class SearchMiniComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.currentFilters) this.loadFilters = true;
  ...
  InitData() {
    ...
    const sources = [this.adminService.GetScreenSearchLocale()];
  ...
  onSelect(index) {
    ...
    this.initSelectionStatus();
    if (value) this.selectionDest[index] = false;
    else this.selectionDest[index] = true;
  }
  ...
  onCheckNav(e: any, compañia: any) {
    ...
    if (e.target.checked == true) {
      ...
      if (
        this.checkedNav.findIndex(
          current_element => current_element.id == "0"
        ) >= 0
      ) {
        this.checkedNav = [];
      }

      this.checkedNav.push({
        id: compañia.id,
        descripcion: compañia.descripcion
      });
      ...
      this.checkedNav.splice(
        this.checkedNav.findIndex(
          current_element => current_element.id == compañia.id
        ),
        1
      );
    ...
    if (compañias.length > 0) {
      ...
      compañias.forEach(newNav => {
        if (this.isCheckNav(newNav)) {
          checkedNavNew.push({
            id: newNav.id,
            descripcion: newNav.descripcion
          });
        }
      });
  ...
  ResetCombos(callCombos: boolean) {
    if (this.filtersInfo) {
  ...
  LoadCombosValues() {
    if (this.filtersInfo) {
      if (this.filtersInfo.destinos.length > 0) {
        if (
          this.currentFilters.destino &&
          this.currentFilters.destino.length > 0
        ) {
          ...
          for (
            let index = 0;
            index < this.filtersInfo.destinos.length;
            index++
          ) {
            ...
            if (destino.hijos && destino.hijos.length > 0) {
              for (
                let indexHijo = 0;
                indexHijo < destino.hijos.length;
                indexHijo++
              ) {
          ...
          if (
            this.filtersInfo.destinos[0].hijos &&
            this.filtersInfo.destinos[0].hijos.length > 0
          ) {
      ...
      if (this.filtersInfo.companias.length > 0) {
        this.checkedNav = [];
        if (
          this.currentFilters.companias &&
          this.currentFilters.companias.length > 0
        ) {
          for (
            let indexFilter = 0;
            indexFilter < this.currentFilters.companias.length;
            indexFilter++
          ) {
            var idCompania = this.currentFilters.companias[indexFilter];
            for (
              let index = 0;
              index < this.filtersInfo.companias.length;
              index++
            ) {
      ...
      if (this.filtersInfo.duraciones.length > 0) {
        if (this.currentFilters.duracion) {
          for (
            let index = 0;
            index < this.filtersInfo.duraciones.length;
            index++
          ) {
      ...
      if (this.filtersInfo.puertos.length > 0) {
        if (
          this.currentFilters.puerto &&
          this.currentFilters.puerto.length > 0
        ) {
          var idPuerto = this.currentFilters.puerto[0];
          for (
            let index = 0;
            index < this.filtersInfo.puertos.length;
            index++
          ) {
  ...
  BuildQueryParams(): any {
    ...
    if (
      this.currentFilters.puerto != null &&
      this.currentFilters.puerto.length > 0
    ) {
      var puerto = "";
    ...
    if (
      this.currentFilters.companias != null &&
      this.currentFilters.companias.length > 0
    ) {
      var companias = "";
      for (
        let index = 0;
        index < this.currentFilters.companias.length;
        index++
      ) {
    ...
    if (
      this.currentFilters.fecha != null &&
      this.currentFilters.fecha != "0-0" &&
      this.currentFilters.fecha != "null"
    ) {
    ...
    if (
      ...
      this.currentFilters.duracion != "0" &&
      this.currentFilters.duracion != "null"
    ) {
